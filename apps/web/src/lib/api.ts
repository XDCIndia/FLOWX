...
function getToken(): string | null {
  ...
  return localStorage.getItem('flowx_api_key');
}
...
class FlowXAPI {
  ...
  async faucet(walletId: string, assetCode = 'USDC', amount = 1000): Promise<{ wallet_id: string; asset_code: string; added: number; new_balance: string; tx_hash?: string }> {
  ...
  // Verify Deposit
  async verifyDeposit(walletId: string, txHash: string): Promise<{ status: string; wallet_id: string; tx_hash: string; amount: string; asset: string }> {
    return request(`/v1/wallets/${walletId}/verify-deposit`, {
      method: 'POST',
