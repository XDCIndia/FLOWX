...
// wallet-to-wallet transfer using FlowX's custody pattern (encrypted
...
//  2. encrypt both private keys with MASTER_ENCRYPTION_KEY (FlowX custody model)
...
func main() {
	...
	// Config: reuse FlowX's .env conventions (viper) so the demo runs with
	...
	fmt.Println("=== FlowX XDC Apothem — wallet-to-wallet transfer model ===")
	...
	// [1] wallets — FlowX custody model: private keys never stored raw,
	// only AES-encrypted with the master key (same as wallet/service.go).
	fmt.Println("\n[1] wallets (FlowX custody: secrets AES-encrypted)")
