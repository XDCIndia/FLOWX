...
// Returns an error if the tx does not exist or has not been mined.
func (c *Client) GetDeposit(ctx context.Context, txHash string) (*DepositInfo, error) {
	x, pending, err := c.ec.TransactionByHash(ctx, common.HexToHash(txHash))
	if err != nil {
		return nil, fmt.Errorf("xdc: fetch tx %s: %w", txHash, err)
	}
	if pending {
		return nil, fmt.Errorf("xdc: tx %s is still pending", txHash)
	}

	receipt, err := c.ec.TransactionReceipt(ctx, common.HexToHash(txHash))
	if err != nil {
		return nil, fmt.Errorf("xdc: receipt %s: %w", txHash, err)
	}

	head, err := c.ec.HeaderByNumber(ctx, nil)
	if err != nil {
		return nil, fmt.Errorf("xdc: head: %w", err)
	}
	...
	// Try multiple signers to handle different transaction types.
	var from common.Address
	var senderErr error
	for _, signer := range []types.Signer{
		types.NewEIP155Signer(c.chain),
		types.NewLondonSigner(c.chain),
		types.NewEIP2930Signer(c.chain),
		types.HomesteadSigner{},
		types.FrontierSigner{},
	} {
		from, senderErr = types.Sender(signer, x)
		if senderErr == nil {
			break
		}
	}
	if senderErr != nil {
		return nil, fmt.Errorf("xdc: derive sender: %w", senderErr)
	}
