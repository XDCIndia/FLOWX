...
func NewService(repo Repository, rail Rail, fxSvc fx.Service, transferSvc transfer.Service, platformWalletID, providerName string, creditAsset string) Service {
	if creditAsset == "" {
		creditAsset = "USDC"
	}
	return &service{
...
func (s *service) InitiateDeposit(ctx context.Context, req DepositRequest) (*DepositResponse, error) {
	// First get a quote for the credit asset to ensure conversion is possible
	// In deposit, user pays Fiat (Source), gets creditAsset (Dest).
	quote, err := s.fxSvc.GetQuote(ctx, req.FiatCurrency, s.creditAsset, req.FiatAmount.String())
	if err != nil {
	...
	deposit := &domain.FiatDeposit{
		...
		USDCAmount:        quote.ToAmount, // amount of creditAsset to credit user
...
func (s *service) InitiateWithdrawal(ctx context.Context, req WithdrawRequest) (*WithdrawResponse, error) {
	...
	// Get live exchange rate from FX service. We fetch the 1-unit quote to get the rate.
	quote, err := s.fxSvc.GetQuote(ctx, s.creditAsset, req.FiatCurrency, "1")
	if err != nil {
	...
	_, err = s.transferSvc.InitiateTransfer(ctx, req.WalletID, s.platformWalletID, s.creditAsset, usdcAmount)
	if err != nil {
...
func (s *service) HandleWebhook(ctx context.Context, payload []byte, signature string) error {
	...
	if evt.Type == EventDepositConfirmed || evt.Type == EventDepositFailed {
		...
		if evt.Status == "completed" {
			if _, err := s.transferSvc.InitiateTransfer(ctx, s.platformWalletID, deposit.WalletID, s.creditAsset, deposit.USDCAmount); err != nil {
				_ = s.repo.UpdateDepositStatus(ctx, deposit.ID, domain.FiatStatusFailed)
		...
		if evt.Status == "completed" {
			...
			_, refundErr := s.transferSvc.InitiateTransfer(ctx, s.platformWalletID, withdrawal.WalletID, s.creditAsset, withdrawal.USDCAmount)
			if refundErr != nil {
