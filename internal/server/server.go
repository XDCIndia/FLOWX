...
func New(
	...
	r.Route("/v1", func(r chi.Router) {
		...
		// Fiat provider webhooks (Flutterwave/Stripe) carry no FlowX auth —
		// they authenticate via per-provider signatures (verif-hash /
		// Stripe-Signature), verified inside the rail. Public + rate-limited,
		// same as /webhooks/verify.
		r.With(webhook.VerifyRateLimit()).Post("/webhooks/fiat/{provider}", fiatHandler.PublicWebhook)
		...
		r.Group(func(r chi.Router) {
			...
			r.Group(func(r chi.Router) {
				...
				r.Route("/payments", routingHandler.Routes())
