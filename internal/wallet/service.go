...
// VerifyDeposit is not supported on the Stellar backend — deposits are
// detected by the indexer's Horizon SSE stream.
func (s *service) VerifyDeposit(_ context.Context, _, _ string) (*domain.Transaction, error) {
	return nil, fmt.Errorf("verify-deposit is only available on the XDC backend")
}
